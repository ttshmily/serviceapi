...
public class UpdateMedicalRecords extends BaseTest {
    ...
    public static String s_UpdateRecords(String token, String orderId, HashMap<String, String> map){
        String res = "";
        TestLogger logger = new TestLogger(s_JobName());
        SurgeryOrder su = new SurgeryOrder("order");
        for(String key:map.keySet()){
            if(JSONObject.fromObject(su.getOrder()).containsKey(key)){
                JSONObject.fromObject(su.getOrder()).replace(key, map.get(key));
            }else {
                JSONObject.fromObject(su.getOrder()).accumulate(key, map.get(key));
            }
        }
    ...
    public void test_01_更新病例_主诉疾病() {
        ...
        s_CheckResponse(res);
        String mdid = Helper.s_ParseJson(data, "order:major_disease_id");
        HashMap<String, String> pathValue = new HashMap<String, String>();
        pathValue.put("orderId", orderId);
        SurgeryOrder su = new SurgeryOrder("order");

        logger.info("更新主诉疾病为-1");
        su.getOrder().setMajor_disease_id("-1");
        res = HttpRequest.s_SendPut(host_doc+uri, su.transform(), userToken, pathValue);
        s_CheckResponse(res);
        ...
        Assert.assertEquals(Helper.s_ParseJson(data, "order:major_disease_id"), mdid, "主诉疾病为-1，不应该更新成功");
        logger.info("禁止更新主诉疾病为-1");


        logger.info("更新主诉疾病为0");
        su.getOrder().setMajor_disease_id("0");
        res = HttpRequest.s_SendPut(host_doc+uri, su.transform(), userToken, pathValue);
        s_CheckResponse(res);
        Assert.assertNotEquals(code, "1000000", "更新异常数据成功");
        Assert.assertEquals(code, "2210416", "更新异常数据成功");
        ...
        s_CheckResponse(res);
        Assert.assertEquals(code, "1000000", "查看订单失败");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:major_disease_id"), mdid, "主诉疾病为0，不应该更新成功");
        logger.info("禁止更新主诉疾病为0");

        logger.info("更新主诉疾病为30000000000");
        su.getOrder().setMajor_disease_id("30000000000");
        res = HttpRequest.s_SendPut(host_doc+uri, su.transform(), userToken, pathValue);
        s_CheckResponse(res);
        Assert.assertNotEquals(code, "1000000", "更新异常数据成功");
        logger.info("查看刚刚更新的订单详情");
        ...
        s_CheckResponse(res);
        Assert.assertEquals(code, "1000000", "查看订单失败");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:major_disease_id"), mdid, "主诉疾病为30000000000，不应该更新成功");
        logger.info("禁止更新主诉疾病为30000000000");

        String key  = Generator.randomKey(KnowledgeBase.kb_disease);
        logger.info("更新主诉疾病为"+key);
        su.getOrder().setMajor_disease_id(key);
        res = HttpRequest.s_SendPut(host_doc+uri, su.transform(), userToken, pathValue);
        s_CheckResponse(res);
        ...
        s_CheckResponse(res);
        Assert.assertEquals(code, "1000000", "查看订单失败");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:major_disease_id"), key, "主诉疾病"+key+"，未更新成功");
        logger.info("更新主诉疾病为"+key+"成功");

        key = Generator.randomKey(KnowledgeBase.kb_disease);
        logger.info("更新主诉疾病为"+key);
        su.getOrder().setMajor_disease_id(key);
        res = HttpRequest.s_SendPut(host_doc+uri, su.transform(), userToken, pathValue);
        s_CheckResponse(res);
    ...
    public void test_02_更新病例_次诉疾病() {
        ...
        res = GetOrderDetail_V1.s_MyInitiateOrder(userToken, orderId);
        s_CheckResponse(res);
        String mdId = Helper.s_ParseJson(data, "order:minor_disease_id");
        HashMap<String, String> pathValue = new HashMap<String, String>();
        pathValue.put("orderId", orderId);
        SurgeryOrder su = new SurgeryOrder("order");

        logger.info("更新次诉疾病为-1");
        su.getOrder().setMinor_disease_id("-1");
        res = HttpRequest.s_SendPut(host_doc+uri, su.transform(), userToken, pathValue);
        s_CheckResponse(res);
        Assert.assertNotEquals(code, "1000000", "更新异常数据成功");
        logger.info("查看刚刚更新的订单详情");
        ...
        s_CheckResponse(res);
        Assert.assertEquals(code, "1000000", "查看订单失败");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:minor_disease_id"), mdId, "次诉疾病为-1，不应该更新成功");
        logger.info("禁止更新次诉疾病为-1");

        logger.info("更新次诉疾病为0");
        su.getOrder().setMinor_disease_id("0");
        res = HttpRequest.s_SendPut(host_doc+uri, su.transform(), userToken, pathValue);
        s_CheckResponse(res);
        Assert.assertNotEquals(code, "1000000", "更新异常数据成功");
        logger.info("查看刚刚更新的订单详情");
        ...
        s_CheckResponse(res);
        Assert.assertEquals(code, "1000000", "查看订单失败");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:minor_disease_id"), mdId, "次诉疾病为0，不应该更新成功");
        logger.info("禁止更新次诉疾病为0");

        logger.info("更新次诉疾病为30000000000");
        su.getOrder().setMinor_disease_id("30000000000");
        res = HttpRequest.s_SendPut(host_doc+uri, su.transform(), userToken, pathValue);
        s_CheckResponse(res);
        Assert.assertNotEquals(code, "1000000", "更新异常数据成功");
        logger.info("查看刚刚更新的订单详情");
        ...
        s_CheckResponse(res);
        Assert.assertEquals(code, "1000000", "查看订单失败");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:minor_disease_id"), mdId, "次诉疾病为30000000000，不应该更新成功");
        logger.info("禁止更新次诉疾病为30000000000");

        String key = Generator.randomKey(KnowledgeBase.kb_disease);
        logger.info("更新次诉疾病为"+key);
        su.getOrder().setMinor_disease_id(key);
        res = HttpRequest.s_SendPut(host_doc+uri, su.transform(), userToken, pathValue);
        s_CheckResponse(res);
        Assert.assertEquals(code, "1000000", "更新正常数据未成功");
        logger.info("查看刚刚更新的订单详情");
        ...
        s_CheckResponse(res);
        Assert.assertEquals(code, "1000000", "查看订单失败");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:minor_disease_id"), key, "次诉疾病"+key+"，未更新成功");
        logger.info("更新次诉疾病为"+key+"成功");

        key = Generator.randomKey(KnowledgeBase.kb_disease);
        logger.info("更新次诉疾病为"+key);
        su.getOrder().setMinor_disease_id(key);
        res = HttpRequest.s_SendPut(host_doc+uri, JSONObject.fromObject(su).toString(), userToken, pathValue);
        s_CheckResponse(res);
        Assert.assertEquals(code, "1000000", "更新正常数据未成功");
        logger.info("查看刚刚更新的订单详情");
        res = GetOrderDetail_V1.s_MyInitiateOrder(userToken, orderId);
    ...
    public void test_03_更新病例_患者性别() {
        ...
        res = GetOrderDetail_V1.s_MyInitiateOrder(userToken, orderId);
        s_CheckResponse(res);
        String gender = Helper.s_ParseJson(data, "order:patient_gender");
        HashMap<String, String> pathValue = new HashMap<String, String>();
        pathValue.put("orderId", orderId);
        SurgeryOrder su = new SurgeryOrder("order");

        logger.info("更新性别为-1");
        su.getOrder().setPatient_gender(-1);
        res = HttpRequest.s_SendPut(host_doc+uri, su.transform(), userToken, pathValue);
        s_CheckResponse(res);
        Assert.assertNotEquals(code, "1000000", "更新异常数据成功");
        logger.info("查看刚刚更新的订单详情");
        ...
        s_CheckResponse(res);
        Assert.assertEquals(code, "1000000", "查看订单失败");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:patient_gender"), gender, "性别为-1，不应该更新成功");

        logger.info("更新性别为0");
        su.getOrder().setPatient_gender(0);
        res = HttpRequest.s_SendPut(host_doc+uri, su.transform(), userToken, pathValue);
        s_CheckResponse(res);
        Assert.assertNotEquals(code, "1000000", "更新异常数据成功");
        logger.info("查看刚刚更新的订单详情");
        ...
        s_CheckResponse(res);
        Assert.assertEquals(code, "1000000", "查看订单失败");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:patient_gender"), gender, "性别为0，不应该更新成功");
        ...
        s_CheckResponse(res);
        Assert.assertEquals(code, "1000000", "查看订单失败");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:patient_gender"), gender, "性别为3，不应该更新成功");

        logger.info("更新性别为2");
        su.getOrder().setPatient_gender(2);
        res = HttpRequest.s_SendPut(host_doc +uri, su.transform(), userToken, pathValue);
        s_CheckResponse(res);
        Assert.assertEquals(code, "1000000", "更新正常数据未成功");
        logger.info("查看刚刚更新的订单详情");
        ...
        s_CheckResponse(res);
        Assert.assertEquals(code, "1000000", "查看订单失败");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:patient_gender"), "2", "性别2未更新成功");
        ...
        s_CheckResponse(res);
        Assert.assertEquals(code, "1000000", "查看订单失败");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:patient_gender"), "1", "性别1未更新成功");
    }
    ...
    public void test_04_更新病例_病例描述() {
        ...
        res = GetOrderDetail_V1.s_MyInitiateOrder(userToken, orderId);
        s_CheckResponse(res);
        String diag = Helper.s_ParseJson(data, "order:diagnosis");
        HashMap<String, String> pathValue = new HashMap<String, String>();
        pathValue.put("orderId", orderId);
        //MedicalRecordsOld mr = new MedicalRecordsOld(true);
        SurgeryOrder su = new SurgeryOrder("order");

        logger.info("更新病例描述");
        su.getOrder().setDiagnosis( "思考房价的问题，关键在于逻辑。\n" +
        ...
        s_CheckResponse(res);
        Assert.assertEquals(code, "1000000", "查看订单失败");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:diagnosis"), "思考房价的问题，关键在于逻辑。\n" +
                "但中国房地产市场最缺的恰恰是逻辑，人们对房价的判断，大多基于一种群羊效应，价格上涨的时候追涨，价格下跌的时候抛售。\n" +
                "以本轮房地产的周期为例，本轮房地产的反转行情最早始于2014年9月30日，在全国房价开始全面下跌之时，中央政府开始出台救市政策，继而在2015年“3.30”房地产政策回到2008年救市模式，在这种情况下，基于对中国房价走势与政策周期之间强烈的相关性，以及房地产市场基本面的判断，我们认为，在房地产基本面已经发生变化的情况下，一线城市、热点城市、大城市需求持续旺盛，其实不存在库存问题，在各种政策的红利下，我们认为买房的时间窗口已经打开，呼吁大家买房，甚至在一些文章中提出：“如果政府丧心病狂的救市，你一定要同样丧心病狂出手买房”的极端话语，但在市场低迷的情况下，无论你如何展示房价的逻辑，市场的参与者是不会轻易出手买房的。同样的道理，在市场陷入疯狂模式之后，动物精神远超人的理性，各种投机因素将使市场的表现大大超越任何原理和规律的范畴。这轮房价的上涨我们有预判，但上涨如此迅猛，持续的时间如此之长，甚至在去年“9.30”之后，在高层三令五申“房子是住的，不是炒的”基调下，一些热点城市仍然疯狂炒作，逼迫政策不断加码。但市场陷入房价不会下跌的幻觉的时候，人们只会看到眼前的疯狂，而看不到逻辑在悄然起作用。\n" +
                "我们讲中国房价的逻辑，有三个维度：一是政策的维度，中国房地产市场本质上政策市，市场周期受制于政策周期的影响，房价的表现也受制于政策的变化，在政策一松一紧之间，房价都会出现剧烈的反弹，中国的房地产政策，只有真假之分，没有见效不见效这一说，政策如果动真格，每次都是见效的，只是在下一次反弹的时候，很多人忘记了房价在政策的影响下曾经跌过；二是供需的维度，供需规律是决定中国房地产市场长期趋势的最基本逻辑。经过近20年的发展，住宅市场供需基本平衡，但因为中国大城市与其他城市之间的资源鸿沟，必然出现中小城市供应过度而大城市供应长期不足的矛盾，再加上中国大城市错误的限制人口，以及中国以中小城市为中心的错误的城镇化的方向，人为控制大城市的人口和建设用地，必然导致供需矛盾长期存在。我一直强调，库存不是中国房地产的真问题，真问题是资源错配，是土地制度和城市发展的大方向违背城镇化的基本规律，从而导致出现人为的短缺；三是价格的逻辑，房价的绝对值已经很高，这是不争的事实。价格尽管是一个历史和时空概念。在每一个时点上，衡量价格高低的标准是不一样的，美国房地产市场1940年以来，从长期趋势看，一直是上涨的，但几乎每十年都要经历一次调整。很多人现在经常以1989年人民日报一篇报道来嘲笑那些认为中国房价已经很高的人，当时房价只有1900元，大家惊呼房价太高。其实，在那个时点，按照各个元素分析，1900元一平米的房价的确很高，现在北京房价均价过了6万，如果按照人均收入，房价中位数，房价收入比，租售比等一系列的指标看，目前的房价不仅绝对值处于高位，而且按照很多指标来权衡，泡沫化程度已经很高。\n" +
                "基于以上三个逻辑来判断，我们认为，中国房地产市场正在出现前所未有的变化：政策为什么如此严厉？一轮又一轮的重拳在传导什么信号？北京为什么对商住房赶尽杀绝？厦门等热点城市为什么出现了“两年之内不能卖出”的限卖规定，一些城市在一个月内甚至不断出台加码的政策，如果对这一系列的信号仍然置若罔闻，一定会犯非常严重的错误。目前政策信号最清楚不过的表明，高层对房地产泡沫风险的担心超过了任何时候：包括张高丽副总理、周小川行长及发改委主任何立峰最近都公开表态，要警惕房地产泡沫风险。张高丽在“中国高层发展论坛”上表示，坚持“房子是用来住的，不是用来炒的”的定位，分类指导，因城施策，重点消化三四线城市房地产的过量库存。并把房地产泡沫列为今年主要的风险点；周小川最近表示，中国金融体系总体健康，但也存在总体杠杆率偏高、债市房市风险和跨市场影子银行业务活跃等风险。国家发改委主任何立峰在“中国高层发展论坛”上将房地产和实体经济的失衡列为当前中国经济发展面临三大结构性的失衡之一，指出“有大量资金涌入房地产市场，曾经一度带动了一线城市和热点二线城市的房价过快的上涨，进一步推高了实体经济发展的成本。”提出要控制信贷资金过度流向房地产业。高层如此密集的对房地产泡沫进行表态，极为罕见，后面透露出的政策信号不容小觑。而且我们看到，高层不仅仅在喊话，而且采取实质性的举措。央行之前加急下发的《做好信贷政策工作的意见》明确要求：人民银行各分支机构要加强对商业银行窗口指导，督促其优化信贷结构，合理控制房贷比和增速，有效防控信贷风险。收紧杠杆，断粮的决心非常坚决。\n" +
    ...
    public void test_05_更新病例_更新图片() {
        ...
        logger.info("新增一张图片。。。");
        su.getOrder().setMedical_record_pictures(new ArrayList<SurgeryOrder.Picture>(){
            {
                add(su.new Picture("2017/05/04/1265834e-97d8-44a0-95e7-047c7facaee8/IMG_20170429_102737.jpg", "1"));
                add(su.new Picture("2017/05/04/1315bbe0-2836-4776-8216-ec55044f32dd/IMG_20161013_172442.jpg", "1"));
                add(su.new Picture("2017/05/04/4ab279ba-4626-4491-abee-25029d2341d6/WechatIMG560.jpeg", "1"));
            }
        });
        res = HttpRequest.s_SendPut(host_doc + uri, su.transform(), userToken, pathValue);
        s_CheckResponse(res);
        ...
        Assert.assertNotNull(Helper.s_ParseJson(data, "order:medical_record_pictures(0):url"), "没有图片URL");
        ...
        Assert.assertNotNull(Helper.s_ParseJson(data, "order:medical_record_pictures(1):url"), "没有图片URL");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:medical_record_pictures(2):key"), "2017/05/04/4ab279ba-4626-4491-abee-25029d2341d6/WechatIMG560.jpeg");
        Assert.assertNotNull(Helper.s_ParseJson(data, "order:medical_record_pictures(2):url"), "没有图片URL");

        logger.info("再新增一张图片。。。");
        su.getOrder().setMedical_record_pictures(new ArrayList<SurgeryOrder.Picture>(){
            {
                add(su.new Picture("abc.jpg", "1"));
                add(su.new Picture("2017/05/04/1265834e-97d8-44a0-95e7-047c7facaee8/IMG_20170429_102737.jpg", "1"));
                add(su.new Picture("2017/05/04/1315bbe0-2836-4776-8216-ec55044f32dd/IMG_20161013_172442.jpg", "1"));
                add(su.new Picture("2017/05/04/4ab279ba-4626-4491-abee-25029d2341d6/WechatIMG560.jpeg", "1"));
            }
        ...
        s_CheckResponse(res);
        Assert.assertEquals(code, "1000000", "查看订单失败");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:medical_record_pictures(0):key"), "abc.jpg");
        Assert.assertNotNull(Helper.s_ParseJson(data, "order:medical_record_pictures(0):url"), "没有图片URL");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:medical_record_pictures(1):key"), "2017/05/04/1265834e-97d8-44a0-95e7-047c7facaee8/IMG_20170429_102737.jpg");
        Assert.assertNotNull(Helper.s_ParseJson(data, "order:medical_record_pictures(1):url"), "没有图片URL");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:medical_record_pictures(2):key"), "2017/05/04/1315bbe0-2836-4776-8216-ec55044f32dd/IMG_20161013_172442.jpg");
        Assert.assertNotNull(Helper.s_ParseJson(data, "order:medical_record_pictures(2):url"), "没有图片URL");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:medical_record_pictures(3):key"), "2017/05/04/4ab279ba-4626-4491-abee-25029d2341d6/WechatIMG560.jpeg");
        Assert.assertNotNull(Helper.s_ParseJson(data, "order:medical_record_pictures(3):url"), "没有图片URL");


        logger.info("删除二张图片。。。");
        su.getOrder().getMedical_record_pictures().remove(3);
        su.getOrder().getMedical_record_pictures().remove(0);
        res = HttpRequest.s_SendPut(host_doc + uri, su.transform(), userToken, pathValue);
        s_CheckResponse(res);
        Assert.assertEquals(code, "1000000", "更新正常数据未成功");
        logger.info("查看刚刚更新的订单详情");
        ...
        logger.info("删除所有图片。。。");
        su.getOrder().getMedical_record_pictures().clear();
        res = HttpRequest.s_SendPut(host_doc + uri, su.transform(), userToken, pathValue);
        s_CheckResponse(res);
        Assert.assertEquals(code, "1000000");
        res = GetOrderDetail_V1.s_MyInitiateOrder(userToken, orderId);
        s_CheckResponse(res);
        Assert.assertEquals(Helper.s_ParseJson(data, "order:medical_record_pictures()"), "0");
    ...
    public void test_06_更新病例_期望手术医院() {
        ...
        res = GetOrderDetail_V1.s_MyInitiateOrder(userToken, orderId);
        s_CheckResponse(res);
        String expectedSurgeryHospitalId = Helper.s_ParseJson(data, "order:expected_surgery_hospital_id");
        String expectedSurgeryHospitalName = Helper.s_ParseJson(data, "order:expected_surgery_hospital_name");
        HashMap<String, String> pathValue = new HashMap<String, String>();
        pathValue.put("orderId", orderId);
        SurgeryOrder su = new SurgeryOrder("order");
        ...
        s_CheckResponse(res);
        Assert.assertEquals(code, "1000000", "查看订单失败");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:expected_surgery_hospital_id"), expectedSurgeryHospitalId, "期望手术医院未更新成功");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:expected_surgery_hospital_name"), expectedSurgeryHospitalName, "期望手术医院未更新成功");
        ...
        s_CheckResponse(res);
        Assert.assertEquals(code, "1000000", "查看订单失败");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:expected_surgery_hospital_id"), "0", "期望手术医院未更新成功");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:expected_surgery_hospital_name"), "待回访", "期望手术医院未更新成功");
        ...
        s_CheckResponse(res);
        Assert.assertEquals(code, "1000000", "查看订单失败");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:expected_surgery_hospital_id"), key, "期望手术医院未更新成功");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:expected_surgery_hospital_name"), Generator.hospitalName(key), "期望手术医院未更新成功");
    }
    ...
    public void test_07_更新病例_患者姓名() {
        ...
        res = GetOrderDetail_V1.s_MyInitiateOrder(userToken, orderId);
        s_CheckResponse(res);
        String name = Helper.s_ParseJson(data, "order:patient_name");
        SurgeryOrder su = new SurgeryOrder("order");
        ...
        s_CheckResponse(res);
        Assert.assertEquals(code, "1000000", "查看订单失败");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:patient_name"), "大头猪", "patient_name未更新");
        ...
        s_CheckResponse(res);
        Assert.assertEquals(code, "1000000", "查看订单失败");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:patient_name"), "方超xyz", "patient_name未更新");
        ...
        s_CheckResponse(res);
        Assert.assertEquals(code, "1000000", "查看订单失败");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:patient_name"), "方超xyz惺惺惜惺惺想寻寻寻寻寻寻寻寻寻寻寻寻寻寻寻寻寻寻寻寻寻寻寻寻寻寻寻寻寻寻寻寻寻寻", "patient_name未更新");
    ...
    public void test_08_更新病例_患者年龄() {
        ...
        res = GetOrderDetail_V1.s_MyInitiateOrder(userToken, orderId);
        s_CheckResponse(res);
        String age = Helper.s_ParseJson(data, "order:patient_age");
    ...
    public void test_09_仅仅更新病例图片() {
        ...
        String res = "";
        logger.info("创建一个新订单");
        //String orderId = CreateOrder.s_CreateOrder(mainToken, new OrderDetailOld(true));
        String orderId = CreateOrder.s_CreateOrder(userToken);
        if (orderId.isEmpty()) {
            logger.error("创建订单失败");
            fail();
        ...
        mr.put("order", order);
        res = HttpRequest.s_SendPut(host_doc + uri, mr.toString(), userToken, pathValue);
        s_CheckResponse(res);
        ...
        Assert.assertEquals(code, "1000000", "查看订单失败");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:medical_record_pictures(0):key"), "2017/05/04/1265834e-97d8-44a0-95e7-047c7facaee8/IMG_20170429_102737.jpg");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:medical_record_pictures(1):key"), "2017/05/04/1265834e-97d8-44a0-95e7-047c7facaee8/IMG_20170429_102738.jpg");
    }
    ...
    public void test_10_更新二次手术字段() {
        ...
        s_CheckResponse(res);
        Assert.assertEquals(code, "1000000", "查看订单失败");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:is_reoperation"), "true");
    }
    ...
    public void test_11_更新患者电话字段() {
        ...
        s_CheckResponse(res);
        Assert.assertEquals(code, "1000000", "查看订单失败");
        Assert.assertEquals(Helper.s_ParseJson(data, "order:patient_phone"), "13899991235");
    }
