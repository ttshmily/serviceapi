...
public class GetOrderList extends BaseTest{
    ...
    public void test_01_获取订单列表_登录用户() {
        ...
        Assert.assertNotNull(Generator.parseJson(data,"order():patient_gender_text"), "次诉疾病ID字段缺失");
        Assert.assertNotEquals(Generator.parseJson(data,"order():order_number"), "", "订单ID字段缺失");
    ...
    public void test_05_获取订单列表_三方通话成功后病历上展示上级医生信息() {
        ...
        Assert.assertNotEquals(Generator.parseJson(data,"order():status"), "", "订单状态字段缺失");
        Assert.assertNotEquals(Generator.parseJson(data,"order():OrderStatusText"), "", "订单状态描述字段缺失");
        Assert.assertNotNull(Generator.parseJson(data,"order():surgeon_id"), "手术医生ID字段不能缺失");
        Assert.assertNotNull(Generator.parseJson(data,"order():surgeon_name"), "手术医生姓名字段不能缺失");
        Assert.assertNotNull(Generator.parseJson(data,"order():surgeon_hospital"), "手术医生所在医院字段不能缺失");
        Assert.assertNotNull(Generator.parseJson(data,"order():surgeon_medical_title"), "手术医生技术职称字段不能缺失");
        Assert.assertNotNull(Generator.parseJson(data,"order():surgeon_academic_title"), "手术医生学术职称字段不能缺失");
    }
}
